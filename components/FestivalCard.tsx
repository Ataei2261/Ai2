...
import { Calendar, Edit, Trash2, FileText, Tag, Clock, Image as LucideImage, Link as LinkIcon, Maximize, ChevronDown, ChevronUp, Target, Download, Brain, Zap, ExternalLink, AlertCircle, UploadCloud, CameraOff, Info as InfoIcon, Star, ListChecks, Layers, MessageSquare, Edit3, FilePlus, XCircle, RefreshCw, FileText as FileTextIcon, CheckSquare, Square, DollarSign } from 'lucide-react'; // Added DollarSign
...
const SmartAnalysisDisplay = ({ analysisString }: { analysisString: string }) => {
    try {
        const parsed = JSON.parse(analysisString);
        if (parsed && typeof parsed === 'object' && 'comprehensiveAnalysis' in parsed) {
            const sections = [
                { title: "تحلیل جامع جشنواره و سوابق", content: parsed.comprehensiveAnalysis },
                { title: "تحلیل داوران", content: parsed.judgesAnalysis },
        ...
        // Not JSON or wrong format, fall through to old string rendering
    }
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col transition-all duration-300 ease-in-out hover:shadow-xl">
        ...
        {isOpen && (
          <div id={`festival-details-${festival.id}`} className="px-3 pt-2 pb-3 border-t border-gray-200 space-y-3">
            ...
            {isAdmin && (
              <>
                {/* Smart Analysis Section (itself collapsible) */}
                    ...
                    {isSmartAnalysisOpen && (
                        <div id={`smart-analysis-${festival.id}`} className="mt-2 space-y-2 text-xs">
                            ...
                            {festival.smartAnalysis && !festival.isAnalyzing && (
                                <>
                                    <SmartAnalysisDisplay analysisString={festival.smartAnalysis} />
                            ...
                            {festival.analysisSourceUrls && festival.analysisSourceUrls.length > 0 && !festival.isAnalyzing && (
                                <div className="mt-1.5 p-1.5 bg-gray-50 rounded border border-gray-100"><p className="text-2xs font-medium text-gray-500 mb-0.5">منابع:</p><ul className="space-y-0.5">{festival.analysisSourceUrls.map((source, index) => (<li key={index} className="text-2xs"><a href={source.uri} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 hover:underline flex items-center" title={source.uri}><ExternalLink size={10} className="me-1 flex-shrink-0" /><span className="truncate">{source.title || source.uri}</span></a></li>))}</ul></div>
                            )}
                            {!festival.smartAnalysis && !festival.isAnalyzing && !festival.analysisError && (<button onClick={handleFetchSmartAnalysis} className="w-full mt-1.5 px-3 py-1 bg-purple-500 text-white font-semibold rounded-md shadow-xs hover:bg-purple-600 flex items-center justify-center text-2xs"><Zap size={12} className="me-1" /> دریافت تحلیل</button>)}
                        </div>
                    )}
                </div>

                {/* Image Analysis Section (itself collapsible) */}
                {festival.smartAnalysis && !festival.isAnalyzing && (
                    <div className="pt-2 mt-2 border-t border-gray-100">
                        <div 
                            className="flex justify-between items-center cursor-pointer py-1"
        ...
        <div className="px-3 py-1.5 bg-gray-50 border-t border-gray-200 flex justify-end items-center space-s-1.5 mt-auto">
          {isAdmin && (
            <button
